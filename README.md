...
- **התקנה כאפליקציה (PWA)** — אייקון במסך הבית, פתיחה במסך מלא והתראות דחיפה. באנדרואיד ובדסקטופ
  הכרטיס מפעיל את ההתקנה של הדפדפן בלחיצה; באייפון, שבו Safari לא מציע כלום מעצמו, הוא מראה את שני
  הצעדים בתפריט השיתוף (`public/js/install.js`)
